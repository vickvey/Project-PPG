...
class PPGFeatureExtractor:
    ...
    def load_and_clean_data(self, file_path) -> pd.DataFrame:
        try:
    ...
    def process_ppg_signals(self, csv_file_path, P_Id):
        ...
        if 'PPG' in df.columns and 'time' in df.columns:
            ...
            if sampling_rate == -1:
                logging.warning(f"Invalid sampling rate for participant {P_Id}. Skipping...")
            ...
            try:
                ...
                logging.error(f"Error processing PPG signals for participant {P_Id}: {e}")
                self.problematic_participants.append(P_Id)
                return None
        else:
            logging.warning(f"'PPG' or 'time' columns not found in {csv_file_path}. Skipping participant {P_Id}...")
            self.problematic_participants.append(P_Id)
            return None
        ...
        for folder_name in os.listdir(self.base_dir):
            if folder_name.endswith('_GSR'):
                ...
                if os.path.isfile(csv_file_path):
                    ...
                    logging.warning(f"File not found: {csv_file_path}. Skipping participant {P_Id}...")
                    self.problematic_participants.append(P_Id)
